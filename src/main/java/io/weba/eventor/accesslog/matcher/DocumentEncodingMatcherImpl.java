...
public class DocumentEncodingMatcherImpl implements Matcher {
    @Override
    public void match(AccessLog accessLog) throws EventorException {
        accessLog.builder.payload.put(
                Output.DOCUMENT_ENCODING.name,
                accessLog.parameters.getOrDefault(Input.DOCUMENT_ENCODING.name, null)
        );
    }
}
