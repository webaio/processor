package io.weba.eventor.infrastructure.log;

import io.weba.eventor.domain.http.Method;
import io.weba.eventor.domain.log.Entry;
import io.weba.eventor.infrastructure.log.gson.GsonFactory;
import io.weba.eventor.infrastructure.log.gson.reader.GsonReader;
import org.joda.time.DateTime;
import org.junit.Test;

import java.util.Base64;

import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertTrue;

public class GsonReaderTest {
    @Test
    public void itShouldReadRequestFromValidJsonLog() throws Exception {
        String accessLog = "{\"request\":{\"id\" : \"7754e3f0-9320-48cc-ae95-fcc8049aec5b\",\"uri\" : \"http:\\/\\/webio.tracker/?t=518de582-e8f1-44e7-8050-cb25cfcb22db&u=http%3A%2F%2Fexample.com%2Fmypage%3Fparam%3Dtrue%23page1&w=1280&h=1024&d1=1&e=1\", \"method\":\"GET\",\"headers\":\"\",\"content\":\"\",\"date\":\"2016-03-05T21:02:40+00:00\",\"remote_addr\":\"100.0.0.1\",\"x_forwarded_for\":\"-\"},\"response\":{\"status\":\"200\", \"headers\":\"\"}, \"localization\" : {\"country\" : \"USA\", \"region\" : \"MA\", \"city\" : \"Northborough\", \"postal_code\" : \"01532\", \"continent\" : \"NA\", \"latitude\" : \"42.3218\", \"longitude\" : \"-71.6350\"}}";
        GsonReader reader = new GsonReader(GsonFactory.create());

        Entry entry = reader.read(accessLog);

        assertEquals("7754e3f0-9320-48cc-ae95-fcc8049aec5b", entry.request.id);
        assertEquals("100.0.0.1", entry.request.IP);
        assertEquals(Method.GET, entry.request.method);
        assertEquals(200, entry.response.statusCode);
        assertTrue(entry.request.headers.bag.size() > 0);
        assertTrue(entry.response.headers.bag.size() > 0);
        assertEquals(new DateTime("2016-03-05T21:02:40+00:00").toDate(), entry.request.date);
        assertEquals(
                new String(Base64.getDecoder().decode("")),
                entry.request.content
        );

        assertEquals("USA", entry.localization.country);
        assertEquals("MA", entry.localization.region);
        assertEquals("Northborough", entry.localization.city);
        assertEquals("01532", entry.localization.postalCode);
        assertEquals("NA", entry.localization.continent);
        assertEquals("42.3218", entry.localization.latitude);
        assertEquals("-71.6350", entry.localization.longitude);
    }

    @Test(expected = NullPointerException.class)
    public void itShouldNotReadRequestFromInvalidJsonLog() throws Exception {
        GsonReader reader = new GsonReader(GsonFactory.create());

        String accessLog = "{\"localization\" : {}}";
        Entry entry = reader.read(accessLog);
    }
}